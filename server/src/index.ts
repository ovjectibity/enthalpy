...
agentchat.on("connection", (socket) => {
  ...
  socket.on("activate_thread",async (msg: ThreadActivation) => {
    ...
    aserv.registerOutputCallback(agentName,
      ...
      aserv.registerModelProvidedStoreCallback(agentName,
        (productContexts: Contexts<ProductContextO>): Promise<void> => {
          console.log("Storing workflow context or asset generated by the agent");
          let toAddContexts = productContexts.contexts.map((ctx: ProductContextO) => {
            return {
              index: -1,
              userId: 1,
              projectId: 1,
          ...
          ProductContextService.addProductContexts(toAddContexts);
        });
